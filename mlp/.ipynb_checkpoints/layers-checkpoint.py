...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        x = np.asarray(inputs)
        if x.ndim < 2:
            raise ValueError("Expected at least 2D input: (batch, input_dim[, ...]).")
    
        # Deterministic/test pass for ORIGINAL dropout: scale by p, no 1/p anywhere
        if not stochastic:
            return x * self.incl_prob
    
        # ----- Stochastic/train pass: element-wise mask over all non-batch axes -----
        # shape of everything except the batch axis
        core_shape = x.shape[1:]            # e.g. (D,), (D,H,W), (D,T), ...
        if self.share_across_batch:
            # one mask shared by the whole batch, but varying across all core dims
            base = (self.rng.rand(*core_shape) <= self.incl_prob)
            mask = base[None, ...]          # add batch axis -> (1, *core_shape)
            mask = np.broadcast_to(mask, x.shape)
        else:
            # per-sample mask, fully element-wise
            mask = (self.rng.rand(*x.shape) <= self.incl_prob)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        if self._mask is None:
            raise RuntimeError("bprop called before a stochastic fprop; run fprop(..., stochastic=True) first.")
            
        return np.asarray(grads_wrt_outputs) * self._mask
